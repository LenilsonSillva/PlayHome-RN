...
export function useOfflineCryptography() {
  ...
  const startGame = useCallback((players: CryptoPlayer[], config: CryptoConfig, manualAssignments?: Record<string, number>) => {
    dispatch({ type: "START_GAME", players, config, manualAssignments });
  },[]);
